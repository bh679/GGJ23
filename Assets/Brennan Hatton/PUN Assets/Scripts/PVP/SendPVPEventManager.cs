...
namespace BrennanHatton.Networking.Events
{
	...
	public class SendPVPEventManager : MonoBehaviourPunCallbacks
	{
		...
		public static void SendUpdateHealthEvent(int id, int targetPlayerId, int damage, string item)
		{
			Debug.Log("SendUpdateHealthEvent id:" + id + " targetPlayerId:" + targetPlayerId+" damage:" +damage);
			object[] content = new object[] { id, targetPlayerId, damage, item }; // Array contains the target position and the IDs of the selected units
